...
pickup or short-distance vendor delivery. It is deliberately not a
citywide or radius-based marketplace — vendors and customers using the
Service are expected to be genuinely local to each other. We do not
...
- You are solely responsible for the accuracy of your shop details, item
  ...
  up to date.
- You are responsible for holding any license, permit, or registration
  required by local law to sell what you list (for example, a barangay
  business permit or a health/sanitation clearance if you sell food).
  We do not verify these unless we say otherwise (see "verified"
  badges below).
- A "verified" status on your vendor profile reflects whatever checks we
  say it reflects at the time — it is not a guarantee of quality,
  legality, or reliability, and we may change what verification requires.
- You set your own payment instructions and payment QR code for your
  shop (for example, a GCash number or bank details). **You are solely
  responsible for the accuracy and security of the payment details you
  publish.** We display what you provide; we do not generate, validate,
  or guarantee it.
- You control your shop's order acceptance and status transitions. Marking
  an order's payment status is your own assertion based on what you've
  seen (for example, a payment screenshot in chat) — we do not verify
  payment and are not responsible if that assertion turns out to be wrong.
...
phase or planned for any future phase. Payment happens directly between
customer and vendor, off-platform, using whatever method the vendor
publishes (bank transfer, e-wallet such as GCash, cash on pickup, etc.).

Consequences of this:

- We are not a party to the payment, do not take a cut of it, and cannot
  reverse, refund, or guarantee it.
- "Payment status" shown in the app is the vendor's own assertion, not a
  fact we've independently verified.
- Any dispute about payment — non-payment, wrong amount, a scam, a failed
...
delivery, sharing proof of payment, asking questions). It is not read or
parsed by the app to make decisions — order status only ever changes
through explicit actions, never by us interpreting what you typed.
Messages, including any image you attach, are a permanent record tied to
the order and visible to the other party in that order for as long as
the order data is retained (see the Privacy Policy).
...
payment or favors in exchange for a rating, and do not post a rating for
...
non-exclusive, royalty-free license to store, display, and resize that
content as needed to operate the Service (for example, generating image
variants). You confirm you have the right to upload what you upload, and
that it doesn't infringe anyone else's rights.

Uploaded images must be JPEG, PNG, or WebP, under 5 MB, and within the
per-item/per-shop/per-message limits shown in the app. Do not upload
...
- List or sell anything illegal, stolen, counterfeit, or requiring a
  license you don't hold;
- Use the Service to harass, threaten, defraud, or impersonate anyone;
- Attempt to bypass rate limits, security controls, or verification;
- Scrape, reverse-engineer, or use the Service to build a competing
  product without our written permission;
- Use another person's account or payment details without authorization;
- Circumvent order/chat records to hide a transaction from platform
...
reasons, or extended inactivity. You may stop using the Service and
...
particular quality, and we do not guarantee that any customer will pay
or that any payment claim is accurate. We are not responsible for the
...
we'll make a reasonable effort to notify you (for example, in-app or by
