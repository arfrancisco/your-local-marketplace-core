...
// Matches Order::VENDOR_CANCELLATION_REASONS on the backend (app/models/order.rb)
// — kept in sync by hand, same convention as OrderStatus in api/types.ts
// rather than fetched from an endpoint, since this is a short, rarely-changing list.
const REASONS: { value: CancellationReasonCode; label: string }[] = [
  { value: 'item_unavailable', label: 'Item(s) no longer available' },
  { value: 'unable_to_fulfill', label: 'Unable to fulfill in time' },
  { value: 'customer_unreachable', label: 'Customer unreachable' },
  { value: 'emergency_closure', label: 'Shop closing early / emergency' },
...
export function CancelOrderModal({ orderId, onClose, onCancelled }: Props) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setSubmitting(true)
    try {
      const res = await api.transitionOrder(orderId, 'cancelled', { reason_code: reasonCode, reason: reason.trim() || undefined })
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" role="dialog" aria-modal="true" aria-label="Cancel order" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={onSubmit}>
          ...
          <p className="muted">Let the customer know why — this also helps us improve.</p>
          <label>
            Reason
