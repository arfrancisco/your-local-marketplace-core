...
- **Behavioral changes without matching e2e coverage.** If the diff
  ...
  it, not just unit tests. This repo has a real pattern of shipping
  behavior changes with only unit coverage updated; flag the gap even if
  unit coverage looks solid otherwise. (Doc currency for the same change —
  `docs/architecture.md`, `docs/manual/` — is the `docs` agent's checklist,
  not this one; if you happen to notice stale docs while reviewing, still
  worth a mention, just don't block a review on it.)
